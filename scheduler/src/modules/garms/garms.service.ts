  ...
  async upload(fileName: string, recordLength: number, remoteFilePath: string) {
    ...
    if (username && password) {
      ...
      const options: FTPS.FTPOptions = {
        ...
        additionalLftpCommands: `set cache:enable no;set ftp:passive-mode on;set ftp:port-range 21-21;set ftp:use-size no;set ftp:ssl-protect-data yes;set ftp:ssl-force yes;set ftps:initial-prot "P";set net:connection-limit 1;set net:max-retries 1;debug 4;`,
      };
