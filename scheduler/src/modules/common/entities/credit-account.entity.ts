...
import {
  ...
  @ApiProperty({
    ...
    @Column({ name: 'CREDIT_ACCOUNT_STATUS_TYPE', type: 'nvarchar', length: 10, nullable: false })
    creditAccountStatusType: string;
  
    @Column({ name: 'CREDIT_ACCOUNT_TYPE', type: 'nvarchar', length: 10, nullable: false })
    creditAccountType: string;
  
    @Column({ name: 'CFS_PARTY_NUMBER', type: 'int', nullable: true })
    cfsPartyNumber: number;
  
    @Column({ name: 'CREDIT_ACCOUNT_NUMBER', type: 'nvarchar', length: 6, nullable: false })
