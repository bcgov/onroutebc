export const VEHICLES_URL =
  import.meta.env.VITE_API_VEHICLE_URL || envConfig.VITE_API_VEHICLE_URL;

export const POLICY_URL =
  import.meta.env.VITE_POLICY_URL || envConfig.VITE_POLICY_URL;

export const GEOCODER_URL =
  import.meta.env.VITE_BC_GEOCODER_API_URL || envConfig.VITE_BC_GEOCODER_API_URL;

export const GEOCODER_API_KEY =
  import.meta.env.VITE_BC_GEOCODER_API_KEY || envConfig.VITE_BC_GEOCODER_API_KEY;

export const GEOCODER_CLIENT_ID =
  import.meta.env.VITE_BC_GEOCODER_CLIENT_ID || envConfig.VITE_BC_GEOCODER_CLIENT_ID;
