...
export const StaffActAsCompanyAuthWall = ({
  ...
  const { data: companyDetails, isPending: isCompanyDetailLoading } =
    useCompanyInfoDetailsQuery(
      /**
       * Passing 0 if companyIdInContext is available because,
       * the query is then unnecessary and 0 is falsy that disables the query.
       */
      companyIdInContext ? 0 : Number(companyIdFromUrl),
    );
