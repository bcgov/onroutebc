...
export const usePermitVehicleManagement = (companyId: number) => {
  ...
  const allVehiclesFromInventory = useMemo(
    () => [
      ...getDefaultRequiredVal([], powerUnitsData).map((powerUnit) => ({
        ...powerUnit,
        vehicleType: VEHICLE_TYPES.POWER_UNIT,
      })),
      ...getDefaultRequiredVal([], trailersData).map((trailer) => ({
        ...trailer,
  ...
  const powerUnitSubtypeNamesMap = useMemo(
    () =>
      new Map<string, string>(
        getDefaultRequiredVal([], powerUnitSubtypesData).map(
  ...
  const trailerSubtypeNamesMap = useMemo(
    () =>
      new Map<string, string>(
        getDefaultRequiredVal([], trailerSubtypesData).map(
          ({ typeCode, type }) => [typeCode, type],
        ),
      ),
    [trailerSubtypesData],
