...
export const VEHICLES_API = {
  ...
  POWER_UNITS: {
    ALL: (companyId: string | number) => `${POWER_UNIT_BASE_URI(companyId)}`,
    DETAIL: (companyId: string | number, powerUnitId: string) =>
      `${POWER_UNIT_BASE_URI(companyId)}/${powerUnitId}`,
    ADD: (companyId: string | number) => `${POWER_UNIT_BASE_URI(companyId)}`,
  ...
  TRAILERS: {
    ALL: (companyId: string | number) => `${TRAILER_BASE_URI(companyId)}`,
    DETAIL: (companyId: string | number, trailerId: string) =>
      `${TRAILER_BASE_URI(companyId)}/${trailerId}`,
    ADD: (companyId: string | number) => `${TRAILER_BASE_URI(companyId)}`,
