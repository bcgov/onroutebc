...
export function addRuntimeFacts(engine: Engine): void {
  ...
  engine.addFact(PolicyFacts.DaysInPermitYear.toString(), async function (params, almanac) {
 ...
 */
export function transformPermitFacts(permitApplication: PermitApplication): PermitFacts {
  ...
  permitFacts.vehicleIdentificationNumber = permitApplication?.permitData?.vehicleDetails?.vin;
  permitFacts.vehiclePlate = permitApplication?.permitData?.vehicleDetails?.plate;
  permitFacts.vehicleType = permitApplication?.permitData?.vehicleDetails?.vehicleSubType;
