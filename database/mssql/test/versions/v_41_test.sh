...
TEST_41_1_RESULT=$(/opt/mssql-tools18/bin/sqlcmd -U ${USER} -P "${PASS}" -S ${SERVER} -v DB_NAME=${DATABASE} -h -1 -i ${TESTS_DIR}/v_41_1_test.sql | xargs)
...
TEST_41_2_RESULT=$(/opt/mssql-tools18/bin/sqlcmd -U ${USER} -P "${PASS}" -S ${SERVER} -v DB_NAME=${DATABASE} -h -1 -i ${TESTS_DIR}/v_41_2_test.sql | xargs)
...
TEST_41_3_RESULT=$(/opt/mssql-tools18/bin/sqlcmd -U ${USER} -P "${PASS}" -S ${SERVER} -v DB_NAME=${DATABASE} -h -1 -i ${TESTS_DIR}/v_41_3_test.sql | xargs)
...
TEST_41_4_RESULT=$(/opt/mssql-tools18/bin/sqlcmd -U ${USER} -P "${PASS}" -S ${SERVER} -v DB_NAME=${DATABASE} -h -1 -i ${TESTS_DIR}/v_41_4_test.sql | xargs)
