...
TEST_35_1_RESULT=$(/opt/mssql-tools18/bin/sqlcmd -U ${USER} -P "${PASS}" -S ${SERVER} -v DB_NAME=${DATABASE} -h -1 -i ${TESTS_DIR}/v_35_1_test.sql | xargs)
...
TEST_35_2_RESULT=$(/opt/mssql-tools18/bin/sqlcmd -U ${USER} -P "${PASS}" -S ${SERVER} -v DB_NAME=${DATABASE} -h -1 -i ${TESTS_DIR}/v_35_2_test.sql | xargs)
