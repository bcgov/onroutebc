...
TEST_51_1_RESULT=$(/opt/mssql-tools18/bin/sqlcmd -C -U ${USER} -P "${PASS}" -S ${SERVER} -v DB_NAME=${DATABASE} -h -1 -i ${TESTS_DIR}/v_51_1_test.sql | xargs)
...
TEST_51_2_RESULT=$(/opt/mssql-tools18/bin/sqlcmd -C -U ${USER} -P "${PASS}" -S ${SERVER} -v DB_NAME=${DATABASE} -h -1 -i ${TESTS_DIR}/v_51_2_test.sql | xargs)
...
TEST_51_3_RESULT=$(/opt/mssql-tools18/bin/sqlcmd -C -U ${USER} -P "${PASS}" -S ${SERVER} -v DB_NAME=${DATABASE} -h -1 -i ${TESTS_DIR}/v_51_3_test.sql | xargs)
if [[ $TEST_51_2_RESULT -eq 1 ]]; then
