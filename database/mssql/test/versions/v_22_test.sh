...
TEST_22_1_RESULT=$(/opt/mssql-tools18/bin/sqlcmd -C -U ${USER} -P "${PASS}" -S ${SERVER} -v DB_NAME=${DATABASE} -h -1 -i ${TESTS_DIR}/v_22_1_test.sql | xargs)
...
TEST_22_2_RESULT=$(/opt/mssql-tools18/bin/sqlcmd -C -U ${USER} -P "${PASS}" -S ${SERVER} -v DB_NAME=${DATABASE} -h -1 -i ${TESTS_DIR}/v_22_2_test.sql | xargs)
