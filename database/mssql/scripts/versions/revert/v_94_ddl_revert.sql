...
IF OBJECT_ID(N'tps.PROCESS_MIGRATED_TPS_PERMITS', N'P') IS NOT NULL
BEGIN
    DROP PROCEDURE [tps].PROCESS_MIGRATED_TPS_PERMITS
END
GO

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_ORBC_TPS_MIGRATED_PERMITS_NEW_PERMIT_NUMBER' AND object_id = OBJECT_ID(N'tps.ORBC_TPS_MIGRATED_PERMITS'))
BEGIN
 	DROP INDEX [tps].[ORBC_TPS_MIGRATED_PERMITS].[IX_ORBC_TPS_MIGRATED_PERMITS_NEW_PERMIT_NUMBER];
END
GO


IF EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_ORBC_TPS_MIGRATED_PERMITS_PROCESSED' AND object_id = OBJECT_ID(N'tps.ORBC_TPS_MIGRATED_PERMITS'))
BEGIN
 DROP INDEX [tps].[ORBC_TPS_MIGRATED_PERMITS].[IX_ORBC_TPS_MIGRATED_PERMITS_PROCESSED];
END
GO


IF EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_ORBC_TPS_MIGRATED_PERMITS_ERR_MSG' AND object_id = OBJECT_ID(N'tps.ORBC_TPS_MIGRATED_PERMITS'))
BEGIN
 DROP INDEX [tps].[ORBC_TPS_MIGRATED_PERMITS].[IX_ORBC_TPS_MIGRATED_PERMITS_ERR_MSG];
END
GO

IF EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_ORBC_TPS_MIGRATED_PERMITS_PENDING' AND object_id = OBJECT_ID(N'tps.ORBC_TPS_MIGRATED_PERMITS'))
BEGIN
 DROP INDEX [tps].[ORBC_TPS_MIGRATED_PERMITS].[IX_ORBC_TPS_MIGRATED_PERMITS_PENDING];
END
GO

IF COL_LENGTH(N'tps.ORBC_TPS_MIGRATED_PERMITS', N'ERROR_MESSAGE') IS NOT NULL
BEGIN
...
IF COL_LENGTH(N'tps.ORBC_TPS_MIGRATED_PERMITS', N'PROCESSING_START_UTC') IS NOT NULL
BEGIN
    ALTER TABLE [tps].[ORBC_TPS_MIGRATED_PERMITS]
...
ALTER TABLE [tps].[ORBC_TPS_MIGRATED_PERMITS]
    DROP CONSTRAINT [DF_ORBC_TPS_MIGRATED_PERMITS_PROCESSED];
GO
...
CREATE OR ALTER TRIGGER [tps].[ORBC_TPS_MIGRATED_PERMITS_TRG] 
...
BEGIN
	...
	BEGIN
       -- ORV2-5524 - Temporary step to set company_id to null for all migrated permits, as we are not confident in the integrity of the client-company mapping data in TPS. This will allow the permits to be migrated successfully, and we can then run a data fix later to populate company_id for those permits where we have high confidence in the client-company mapping.
       -- Initialize variables to null at the start of each loop iteration to avoid
       SET @company_id = NULL
       SET @legal_name = NULL
...
SET @VersionDescription = 'Reverting updates to ORBC_TPS_MIGRATED_PERMITS_TRG trigger.'

INSERT [dbo].[ORBC_SYS_VERSION] ([VERSION_ID], [DESCRIPTION], [RELEASE_DATE]) VALUES (93, @VersionDescription, getutcdate())
GO
