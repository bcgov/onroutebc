...
WHERE POLICY_CONFIGURATION_ID = (SELECT MAX(POLICY_CONFIGURATION_ID) FROM [dbo].[ORBC_POLICY_CONFIGURATION])
  AND CHANGE_DESCRIPTION = N'ORV2-6026: STOW Remove trailer vehicle sub-type: Pole Trailers from allowable list.'

IF @@ROWCOUNT <> 1
   THROW 50000, 'STOW Remove trailer vehicle sub-type: Pole Trailers from allowable list revert requires the latest policy configuration to belong to ORV2-6026.', 1;
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO

DECLARE @VersionDescription VARCHAR(255)
SET @VersionDescription = 'Revert ORV2-6026: STOW Remove trailer vehicle sub-type: Pole Trailers from allowable list.'

INSERT [dbo].[ORBC_SYS_VERSION] ([VERSION_ID], [DESCRIPTION], [RELEASE_DATE])
VALUES (111, @VersionDescription, GETUTCDATE())
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO
COMMIT TRANSACTION
