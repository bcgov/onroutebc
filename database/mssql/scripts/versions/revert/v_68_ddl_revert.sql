...
WHERE DOCUMENT_ID IN (
		...
		WHERE FILE_NAME = 'mfp-template-v4.docx'
		)
...
WHERE DOCUMENT_ID IN (
		SELECT ID
		FROM [dops].[ORBC_DOCUMENT]
		WHERE FILE_NAME = 'mfp-void-template-v4.docx'
		)
...
WHERE DOCUMENT_ID IN (
		SELECT ID
		FROM [dops].[ORBC_DOCUMENT]
		WHERE FILE_NAME = 'mfp-revoked-template-v4.docx'
		)
...
SET @VersionDescription = 'Revert MFP template fix'
