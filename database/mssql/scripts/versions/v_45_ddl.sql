...
CREATE TABLE [permit].[ORBC_PERMIT_LOA] (
  ...
  [DB_LAST_UPDATE_TIMESTAMP] [datetime2](7) NULL,
    CONSTRAINT [PK_ORBC_PERMIT_LOA_ID] PRIMARY KEY CLUSTERED (
        [PERMIT_LOA_ID] ASC
    ) WITH (
        PAD_INDEX = OFF,
    ...
    CONSTRAINT [CK_ORBC_PERMIT_LOA_UNQ] UNIQUE NONCLUSTERED 
(
	[PERMIT_ID] ASC,
	[LOA_ID] ASC)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
) ON [PRIMARY]
GO
