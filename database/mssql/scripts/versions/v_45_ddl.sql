...
CREATE TABLE [permit].[ORBC_PERMIT_LOA] (
   ...
   CONSTRAINT [PK_ORBC_PERMIT_LOA_ID] PRIMARY KEY CLUSTERED ([PERMIT_LOA_ID] ASC) WITH (
      PAD_INDEX = OFF,
...
EXEC sys.sp_addextendedproperty @name = N'MS_Description',@value = N'Permit Id.',@level0type = N'SCHEMA',@level0name = N'permit',@level1type = N'TABLE',@level1Name = N'ORBC_PERMIT_LOA',@level2type = N'COLUMN',@level2name = N'PERMIT_ID'

EXEC sys.sp_addextendedproperty @name = N'MS_Description',@value = N'LoA Id.',@level0type = N'SCHEMA',@level0name = N'permit',@level1type = N'TABLE',@level1Name = N'ORBC_PERMIT_LOA',@level2type = N'COLUMN',@level2name = N'LOA_ID'
...
ALTER TABLE [permit].[ORBC_PERMIT_LOA] ADD CONSTRAINT [CK_ORBC_PERMIT_LOA_UNQ] UNIQUE (PERMIT_ID,LOA_ID);

IF @@ERROR <> 0
   SET NOEXEC ON
GO

CREATE SEQUENCE [permit].[ORBC_PERMIT_LOA_H_ID_SEQ] AS [bigint] START
...
CREATE TRIGGER ORBC_PERMIT_LOA_A_S_IUD_TR ON [permit].[ORBC_PERMIT_LOA]
FOR INSERT,
   UPDATE,
   DELETE
AS
...
BEGIN TRY
   ...
   IF EXISTS (
      ...
      UPDATE [permit].[ORBC_PERMIT_LOA_HIST]
   ...
   IF EXISTS (
      ...
      INSERT INTO [permit].[ORBC_PERMIT_LOA_HIST] (
         [PERMIT_LOA_ID],
         [PERMIT_ID],
         [LOA_ID],
      ...
      SELECT [PERMIT_LOA_ID],
         ...
         [DB_LAST_UPDATE_TIMESTAMP],
         (
            NEXT value FOR [permit].[ORBC_PERMIT_LOA_H_ID_SEQ]
            ) AS [_PERMIT_LOA_HIST_ID],
