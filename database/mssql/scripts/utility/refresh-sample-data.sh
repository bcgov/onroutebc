...
sqlcmd -U ${USER} -P "${PASS}" -S ${SERVER} -d ${DATABASE} -Q "DELETE FROM dms.ORBC_DOCUMENT"
...
sqlcmd -U ${USER} -P "${PASS}" -S ${SERVER} -d ${DATABASE} -i ${SCRIPT_DIR}/sampledata/dms.ORBC_DOCUMENT.Table.sql
