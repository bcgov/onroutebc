...
sqlcmd -U ${USER} -P "${PASS}" -S ${SERVER} -d ${DATABASE} -Q "DELETE FROM dbo.ORBC_DOCUMENT_TEMPLATE"
...
sqlcmd -U ${USER} -P "${PASS}" -S ${SERVER} -d ${DATABASE} -i ${SCRIPT_DIR}/sampledata/dbo.ORBC_DOCUMENT_TEMPLATE.Table.sql
