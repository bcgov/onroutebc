...
# is running on.
echo "**LEGACY** Reverting database to empty state..."
((NEXTVER=ORBC_DB_VERSION))
echo "**LEGACY** Initial revert file to look for: ${SCRIPT_DIR}/versions/revert/v_${NEXTVER}_ddl_revert.sql"

while test -f "${SCRIPT_DIR}/versions/revert/v_${NEXTVER}_ddl_revert.sql"; do
    ${SCRIPT_DIR}/utility/legacy/revert-db-single.sh -v ${NEXTVER} -u ${USER} -p "${PASS}" -s ${SERVER} -d ${DATABASE}
    ((NEXTVER=NEXTVER-1))
    echo "**LEGACY** Next revert file to check: ${SCRIPT_DIR}/versions/revert/v_${NEXTVER}_ddl_revert.sql"
done

echo "**LEGACY** Finished reverting database to empty state"
