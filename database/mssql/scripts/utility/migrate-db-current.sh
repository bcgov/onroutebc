...
if (( orbc_db_version < orbc_max_db_version )); then
  echo "You are about to migrate the ${DATABASE:-$MSSQL_MOTI_DB} database on ${SERVER:-$MSSQL_MOTI_HOST} as user ${USER:-$MSSQL_MOTI_USER} from version ${orbc_db_version} to version ${orbc_max_db_version}"
  echo_param_usage
