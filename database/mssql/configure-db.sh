...
export PATH="/usr/local/bin:/opt/mssql-tools18/bin:$PATH"
...
while ([[ ${DBSTATUS} -ne 0 ]] || [[ ${ERRCODE} -ne 0 ]]) && [[ ${i} -lt 60 ]]; do
  ...
  ((i=i+1))
  DBSTATUS=$(sqlcmd -h -1 -t 1 -U ${MSSQL_SA_USER} -P ${MSSQL_SA_PASSWORD} -S ${MSSQL_HOST} -Q "SET NOCOUNT ON; Select SUM(state) from sys.databases")
  ERRCODE=$?
  sleep 1
...
sqlcmd -U ${MSSQL_SA_USER} -P "${MSSQL_SA_PASSWORD}" -S ${MSSQL_HOST} -d master -Q "CREATE DATABASE ${MSSQL_DB}"
