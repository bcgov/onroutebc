import { PermitType } from '../enum/permit-type.enum';

export const PERMIT_TYPES_FOR_QUEUE: readonly PermitType[] = [
  PermitType.SINGLE_TRIP_OVERSIZE,
  PermitType.SINGLE_TRIP_OVERWEIGHT,
  PermitType.SINGLE_TRIP_OVERWEIGHT_OVERSIZE_EMPTY,
];

export const STOS_MAX_ALLOWED_DURATION_AMEND = 30;
export const STOW_MAX_ALLOWED_DURATION_AMEND = 30;
export const STWSE_MAX_ALLOWED_DURATION_AMEND = 30;
export const STGVWI_MAX_ALLOWED_DURATION_AMEND = 30;
export const EPTOP_MAX_ALLOWED_DURATION_AMEND = 30;

export const DEFAULT_STAFF_MAX_ALLOWED_START_DATE = 60;

export const MAX_HC_ALLOWED_PAST_DAYS = 60;
