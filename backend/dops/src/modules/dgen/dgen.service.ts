...
import { DownloadMode, DgenReturnType } from '../../common/enum/dgen.enum';
...
export class DgenService {
  ...
  /**
   * Generate dgen document using CDOGS and an inline template
   ...
   * @returns {ArrayBuffer} an Array Buffer of the dgen
   */
  private async createPDF(
    ...
    // Calls the CDOGS service, which converts the the template document into a dgen
    ...
    const dgen = (await cdogsResponse.data) as ArrayBuffer;

    return dgen;
  }

  /**
   * Saves the dgen in S3 using the DMS service
   * @param {string} accessToken
   * @param {ArrayBuffer} dgen
   * @param {DgenReturnType} returnValue optional return value type
   * @returns Defaults to DMS document Id
   */
  private async savePDF(
    accessToken: string,
    dgen: ArrayBuffer,
    returnValue?: DgenReturnType,
  ): Promise<string> {
    // Convert Array Buffer to Blob to pass to DMS microservice
    const formData = new FormData();
    formData.append(
      'file',
      new Blob([dgen], { type: 'application/dgen' }),
      // TODO: provide a name
      'test',
    );
    ...
    switch (returnValue) {
      case DgenReturnType.MIME_TYPE:
        returnVal = dmsResource.objectMimeType;
        break;
      case DgenReturnType.DMS_DOC_ID:
        returnVal = dmsResource.documentId;
        break;
      case DgenReturnType.PRESIGNED_URL:
        returnVal = dmsResource.preSignedS3Url;
        break;
      case DgenReturnType.S3_OBJ_ID:
   ...
   * @param {PermitTemplateData} permit permit data used to populate the .docx template
   * @param {number} templateVersion template version
   * @param {DgenReturnType} returnValue optional parameter to choose the return type
  ...
  public async generatePDF(
    ...
    // Call CDOGS to generate the dgen
    const dgen = await this.createPDF(permit, template);

    // Call DMS to store the dgen
    const dms = await this.savePDF(accessToken, dgen, returnValue);
